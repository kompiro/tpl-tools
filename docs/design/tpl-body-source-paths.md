...
[PR #24](https://github.com/kompiro/tpl-tools/pull/24) のレビュー指摘（1 巡目 4 件、2 巡目 3 件、3 巡目 2 件）に対する結論。いずれも実装とテストに反映済み。どれも「Markdown をどう読むか」の話で、パスについての主張は変えていない。karasu の 138 件に対する判定は指摘前後で完全に一致する。
