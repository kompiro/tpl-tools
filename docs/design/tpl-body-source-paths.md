...
[PR #24](https://github.com/kompiro/tpl-tools/pull/24) のレビュー指摘（1 巡目 4 件、2 巡目 3 件、3 巡目 2 件、4 巡目 1 件）に対する結論。いずれも実装とテストに反映済み。どれも「Markdown をどう読むか」の話で、パスについての主張は変えていない。karasu の 138 件に対する判定は指摘前後で完全に一致する。

- **code span は backtick run の長さで閉じる。** 単一の backtick だけを見ていると、backtick 2 本で囲んだ span はパディングの空白ごと候補になって落ち（見逃し）、逆に内側の backtick を区切りと取って、行が名指していないパスを拾う（誤検知）。CommonMark どおり「n 本の run は次の n 本で閉じる」で読み、両端に空白があれば 1 つずつ外す。backslash escape は解釈しないが、`\` を含む span は候補にならないので見逃し側にしか転ばない。
...
- **span は行ではなく段落単位で読む。** code span は行をまたいで閉じられる。1 行ずつ渡すと、折り返した span の後半がそれ自体 span に見えて、記録が名指していないパスを拾う。karasu の 138 件には行をまたぐ span が実際に 5 つある（いずれも中にパスを含まないので実害はまだ出ていない）。空行・宣言・fence で区切った段落を丸ごと走査し、span の位置から行番号を引く。閉じ相手のない run は CommonMark どおり地の文なので、backtick 1 つの書き損じが以降を黙らせることはない。
- **引用の中の宣言も読む。** 引用の中の散文はパスを検査しているのに、`> <!-- absent-path-next-line: ... -->` は生のままマッチさせていたので宣言だけ効かなかった。引用の中でパスを名指した記録に逃げ場がなくなる（誤検知になる）ので、`>` を外してから宣言を判定する。
- **list item が同じ行で開く fence も fence として扱う。** `- ` の直後に置かれた fence は検出に掛からず、中の実行例が散文として読まれていた。quote marker と同じく、fence 判定の前に list marker を 1 つ落とし、その幅（item の content 列）を fence に持たせる。閉じ区切りはその列に置かれるので、`10. ` のように 4 文字幅の marker でも閉じられる（持たせないと閉じ漏れて以降が黙る）。ただし**インデント 4 文字以上の fence（深い入れ子の list の中身）は今も検出しない**。CommonMark の block 構造を持たない以上ここが境界で、閉じ区切りの判定を緩めると今度は fence の途中で閉じてしまい誤検知が増える。karasu の 138 件の fence は 8 行すべてインデント 0 なので、当面はこの線で足りる。
- **引用の中の fence は深さまで持つ。** `quoted` の真偽だけでは、`> >` で開いた fence が `>` に戻っても開いたままになり、逆に 1 段深い位置の区切りが浅い fence を閉じてしまう。開いたときの深さを持ち、それより浅くなったら閉じ、閉じ判定は自分の深さ分だけ `>` を外してから行う（外した後もまだ `>` が残るなら、それは中身であって閉じ区切りではない）。
