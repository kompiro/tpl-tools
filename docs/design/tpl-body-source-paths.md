...
- **生成物セグメントの一覧を固定にするか、設定可能にするか。** karasu の一覧には `.astro` / `.vscode-test` / `preview-dist` など repo 固有のものが混ざる。まずは普遍的な 5 つ（`node_modules` / `dist` / `out` / `coverage` / `build`）を固定で持ち、足りない例が出てから考える方に倒したい。prefix と違い、ここは誤検知ではなく**見逃し**の側に転ぶので、固定でも安全側である。
