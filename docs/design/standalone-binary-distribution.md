...
- **install スクリプト**: `curl -fsSL https://raw.githubusercontent.com/kompiro/tpl-tools/main/install.sh | sh`
  - `uname` で OS/arch を判定 → 最新 Release（または `TPL_VERSION` 指定）から該当アセットを DL → SHA256 検証 → `~/.local/bin/tpl` に配置・`chmod +x`
- **devcontainer**: `Dockerfile` に install スクリプトの `RUN` を 1 行、または devcontainer feature として導入。adr-tools と並べて 2 ツールを入れる構成を想定
...
- `--config` で渡す設定ファイル（adr.config.json 共有）の解決パスは CWD 基準のままでよいか（バイナリ配布でも問題ないことは検証済みだが、利用ガイドで明示が必要）
- install スクリプトのインストール先デフォルト（`~/.local/bin` か `/usr/local/bin` か）
- adr-tools と tpl-tools を 1 つの install スクリプト / devcontainer feature でまとめて入れられるようにするか
